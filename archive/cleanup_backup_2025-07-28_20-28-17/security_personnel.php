    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_security_personnel':
                ...
                try {
                    $stmt = $pdo->prepare("INSERT INTO admin_users (username, email, password, full_name, role, is_active, created_at) VALUES (?, ?, ?, ?, 'security_personnel', 1, NOW())");
            ...
            case 'update_security_personnel':
                ...
                try {
                    $stmt = $pdo->prepare("UPDATE admin_users SET is_active = ? WHERE id = ? AND role = 'security_personnel'");
            ...
            case 'delete_security_personnel':
                ...
                try {
                    $stmt = $pdo->prepare("DELETE FROM admin_users WHERE id = ? AND role = 'security_personnel'");
...
try {
    $stmt = $pdo->query("SELECT * FROM admin_users WHERE role = 'security_personnel' ORDER BY created_at DESC");
...
try {
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM admin_users WHERE role = 'security_personnel'");
    $total_security = $stmt->fetch()['total'];
    
    $stmt = $pdo->query("SELECT COUNT(*) as active FROM admin_users WHERE role = 'security_personnel' AND is_active = 1");
...
<body>
    <div class="container">
        ...
        <div class="actions">
            ...
            <a href="admin_users.php" class="btn btn-warning">👥 Manage All Users</a>
        </div>
    </div>
</body>
</html> 